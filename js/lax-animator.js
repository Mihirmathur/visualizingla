...
var CAR_COLORS = ['#000000', '#D0021B', '#F8C81C', '#00009F']
var colors = {
    outgoingCar: "#F8C81C",
    incomingCar: "#D0021B",
    planes: "#00009F"
...
function spawnIncomingCar() {
  ...
  var car = d3.select("#together-chart")
                .append('g')
                .attr('class', 'car-in');
  
  car.append('path')
    .attr('fill', colors.incomingCar)
...
function animateOutgoingCars() {
  TweenMax.to(".car-out", 0, {
    ...
    var animation = TweenMax.staggerFrom(".car-out", CAR_TIME, {
      bezier: {
        values: path,
        type: "cubic",
        autoRotate: 90
...
// planes
function spawnPlane() {
    var plane = d3.select("#together-chart")
                    .append('g')
                    .attr('class', 'plane')
...
function animatePlanes() {
    ...
    var path = MorphSVGPlugin.pathDataToBezier("#flightPath", {align: ".plane"});
    ...
    TweenMax.staggerTo(".plane", FLIGHT_TIME, {
        bezier: {
            values: path,
            type: "cubic",
            curviness: 1,
...
$(document).ready(function() {
    setTimeout(function() {
        for (var i = 0; i < NUM_CARS_IN; i++) {
            spawnIncomingCar();
        }
        animateIncomingCars();
        
        for (var i = 0; i < NUM_CARS_OUT; i++) {
            spawnOutgoingCar();
        }
        animateOutgoingCars();
